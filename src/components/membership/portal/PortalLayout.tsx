...
const Sidebar: React.FC<{ onNavigate?: () => void }> = ({ onNavigate }) => {
  ...
  const sidebarTier = mapTier(user?.tier) ?? member.tier;
      ...
      <div className="px-4 pb-5 space-y-3">
        <div className="border border-[#EF9F27]/40 rounded-sm p-3 bg-[#FEF3C7]/5">
          ...
          <div className="flex items-center justify-between">
            <span className="text-white font-bold text-sm">{TIER_LABELS[sidebarTier]}</span>
...
function mapTier(value?: string): MembershipTier | null {
